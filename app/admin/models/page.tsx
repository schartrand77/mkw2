...
type AdminModelsPageProps = {
  searchParams?: Promise<{ q?: string | string[]; modelId?: string | string[] }>
}
...
export default async function AdminModelsPage({ searchParams }: AdminModelsPageProps) {
  ...
  const modelId = Array.isArray(resolvedSearchParams.modelId) ? resolvedSearchParams.modelId[0] : resolvedSearchParams.modelId
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-white/10 bg-black/20 p-5">
        <ModelManager initialQuery={q || ''} initialModelId={modelId || ''} />
      </div>
    </div>
  )
}
