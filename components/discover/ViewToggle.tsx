...
export default function ViewToggle({ viewMode, gridHref, compactHref }: Props) {
  const setPreference = useCallback((nextView: 'grid' | 'compact') => {
    if (typeof document === 'undefined') return
    const maxAge = 60 * 60 * 24 * 365
    document.cookie = `mwv2_discover_view=${nextView}; path=/; max-age=${maxAge}`
  }, [])
