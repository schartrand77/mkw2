...
export default function AdminSidebar({ className = '', onNavigate }: Props) {
  ...
  return (
    <aside className={`admin-sidebar rounded-xl border border-white/10 bg-black/20 p-4 ${className}`.trim()}>
      <div className="admin-sidebar-kicker text-xs uppercase tracking-[0.28em] text-slate-400">Admin</div>
      <nav className="mt-4 space-y-5" aria-label="Admin navigation">
        {ADMIN_NAV_SECTIONS.map((section) => (
          <div key={section.title} className="space-y-2">
            <p className="admin-sidebar-section-title flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">
                ...
                return (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      onClick={onNavigate}
                      className={`admin-sidebar-link block rounded-md px-3 py-2 text-sm transition ${active ? 'admin-sidebar-link-active bg-white/15 text-white' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}
