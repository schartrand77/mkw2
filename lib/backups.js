...
function commandAvailable(cmd) {
  ...
  if (path.isAbsolute(cmd) || cmd.includes("\\") || cmd.includes("/")) {
    ...
    if (shouldUseShell(cmd)) {
      ...
      const result = spawnSync(commandLine, { encoding: "utf8", shell: true, cwd: projectRoot })
      if (result.error) return false
      return result.status === 0
    }
    const result = spawnSync(cmd, ["--version"], { encoding: "utf8", cwd: projectRoot })
    if (result.error) return false
    return result.status === 0
  }
  if (shouldUseShell(cmd)) {
  ...
  if (result.error) {
    if (process.platform === "win32" && !cmd.includes("\\") && !cmd.includes("/")) {
      const commandLine = buildWindowsShellCommand(cmd, ["--version"])
      const shellResult = spawnSync(commandLine, { encoding: "utf8", shell: true, cwd: projectRoot })
...
function localPgDump(destFile, cmd = resolvePgDumpCommand()) {
  ...
  const tryCommand = (candidate) => {
    const useWindowsShell =
      shouldUseShell(candidate) ||
      (process.platform === "win32" && !path.isAbsolute(candidate) && !candidate.includes("\\") && !candidate.includes("/"))
    if (useWindowsShell) {
      const commandLine = buildWindowsShellCommand(candidate, args)
      return spawnWithStdout(commandLine, [], {
        cwd: projectRoot,
        env,
        encoding: "buffer",
        shell: true,
    ...
    return spawnWithStdout(candidate, args, {
      cwd: projectRoot,
      env,
      encoding: "buffer",
    })
  ...
  const candidates = [
    ...
    "pg_dump",
    process.platform !== "win32" ? "/usr/bin/pg_dump" : null,
    ...getLinuxPostgresBinaryCandidates("pg_dump"),
  ]
    .filter(Boolean)
  ...
  for (const candidate of uniqueCandidates) {
    try {
      const stdout = tryCommand(candidate)
      fs.writeFileSync(destFile, stdout)
      if (candidate !== cmd) {
        log(`Primary pg_dump command failed; used fallback '${candidate}'.`)
      }
      return
    } catch (err) {
      lastErr = err
  ...
  throw lastErr || new Error("pg_dump failed")
}

function getLinuxPostgresBinaryCandidates(binaryName) {
  ...
  versions.sort((a, b) => compareVersionNames(b, a))
  return versions.map((version) => path.join(base, version, "bin", binaryName))
}
...
function runSql(sql) {
  ...
  if (
    shouldUseShell(psqlCmd) ||
    (process.platform === "win32" && !path.isAbsolute(psqlCmd) && !psqlCmd.includes("\\") && !psqlCmd.includes("/"))
  ) {
    const commandLine = buildWindowsShellCommand(psqlCmd, psqlArgs)
    spawnWithStdout(commandLine, [], { cwd: projectRoot, env, shell: true })
    return
  }
  spawnWithStdout(psqlCmd, psqlArgs, { cwd: projectRoot, env })
}
...
function restoreSqlFile(sqlFile) {
  ...
  if (
    ...
    const commandLine = buildWindowsShellCommand(psqlCmd, psqlArgs)
    spawnWithStdout(commandLine, [], { cwd: projectRoot, env, input: body, shell: true })
    return
  }
  spawnWithStdout(psqlCmd, psqlArgs, { cwd: projectRoot, env, input: body })
}
