...
function getBackupReadiness() {
  ...
  const mode = compose ? "docker" : pgDumpAvailable ? "local" : "unavailable"
  const reasons = []
  if (!compose) reasons.push("Docker Compose unavailable")
  if (!pgDumpAvailable) reasons.push("pg_dump unavailable")
  return {
    ok: mode !== "unavailable",
    mode,
    dockerComposeAvailable: Boolean(compose),
    pgDumpAvailable,
...
function getRestoreReadiness() {
  ...
  const mode = compose ? "docker" : psqlAvailable ? "local" : "unavailable"
  const reasons = []
  if (!compose) reasons.push("Docker Compose unavailable")
