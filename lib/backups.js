...
function probeDockerClientTool(tool) {
  ...
  if (!compose) {
    return { ok: false, reason: "Docker Compose unavailable", compose: null }
  }
  ...
  if (res.status === 0) {
    return { ok: true, reason: null, compose, service }
  }
  const stderr = (res.stderr || "").trim()
  const stdout = (res.stdout || "").trim()
  const detail = stderr || stdout || `${tool} probe failed`
  return { ok: false, reason: `Docker ${tool} unavailable for service '${service}': ${detail}`, compose, service }
}
...
function getBackupReadiness() {
  ...
  const reasons = []
  if (!docker.ok && docker.reason) reasons.push(docker.reason)
  ...
  if (!tryDockerDump(destFile)) {
    ...
    try {
      ...
      if (/ENOENT/i.test(message)) {
        throw new Error(
          "pg_dump executable was not found. Add PostgreSQL bin directory to PATH or set PG_DUMP_BIN to full path (for example: C:\\Program Files\\PostgreSQL\\16\\bin\\pg_dump.exe).",
        )
      }
      throw err
    }
